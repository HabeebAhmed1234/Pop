...
public class Entity implements IEntity {
  ...
  public void setColor(final float pRed, final float pGreen, final float pBlue,
      final float pAlpha) {
    if (this.mAndengineColor
        .setChecking(pRed, pGreen, pBlue, pAlpha)) { // TODO Is this check worth it?
      this.onUpdateColor();
    }
  }
  ...
  public Transformation getParentToLocalTransformation() {
    ...
    if (this.mParentToLocalTransformationDirty) {
      ...
      if ((scaleX != 1) || (scaleY != 1)) {
        ...
        parentToLocalTransformation.postScale(1 / scaleX, 1
            / scaleY); // TODO Division could be replaced by a multiplication of 'scale(X/Y)Inverse'...
        parentToLocalTransformation.postTranslate(scaleCenterX, scaleCenterY);
      }
  ...
  private void assertEntityHasNoParent(final IEntity pEntity) throws IllegalStateException {
    if (pEntity.hasParent()) {
      ...
      throw new IllegalStateException(
          "pEntity '" + entityClassName + "' already has a parent: '" + currentParentClassName
