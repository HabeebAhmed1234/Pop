...
public class ParticleSystem<T extends IEntity> extends Entity {
  ...
  public ParticleSystem(final IEntityFactory<T> pEntityFactory,
      final IParticleEmitter pParticleEmitter, final float pRateMinimum, final float pRateMaximum,
      final int pParticlesMaximum) {
    this(0, 0, pEntityFactory, pParticleEmitter, pRateMinimum, pRateMaximum, pParticlesMaximum);
  }

  @SuppressWarnings("unchecked")
  public ParticleSystem(final float pX, final float pY, final IEntityFactory<T> pEntityFactory,
      final IParticleEmitter pParticleEmitter, final float pRateMinimum, final float pRateMaximum,
      final int pParticlesMaximum) {
    super(pX, pY);
...
//		particles[this.mParticlesAlive] = particle;
  ...
  /**
   * removes all particle initializers of the given type
   */
  ...
  /**
   * removes all particle modifiers of the given type
   */
  public void removeParticleModifiers(final Class type) {
    Iterator<IParticleModifier<T>> it = mParticleModifiers.iterator();
    while (it.hasNext()) {
      IParticleModifier<T> modifier = it.next();
  ...
  private void spawnParticles(final float pSecondsElapsed) {
    ...
    final int particlesToSpawnThisFrame = Math.min(this.mParticlesMaximum - this.mParticlesAlive,
        (int) Math.floor(this.mParticlesDueToSpawn));
