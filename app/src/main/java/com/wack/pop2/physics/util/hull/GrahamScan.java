...
public class GrahamScan extends BaseHullAlgorithm {
	...
	private void grahamScan() {
		...
		final Vec2 pl = new Vec2(this.mVertices[0]);
		this.makeAllVerticesRelativeTo(pl);
		this.sort();
		this.makeAllVerticesRelativeTo(new Vec2(pl).mul(-1));
		int i = 3;
		int k = 3;
	...
	private void makeAllVerticesRelativeTo(final Vec2 pVector) {
		final Vec2[] vertices = this.mVertices;
		final int vertexCount = this.mVertexCount;
		
		final Vec2 vertexCopy = new Vec2(pVector); // necessary, as pVector might be in mVertices[]
	...
	private boolean isConvex(final int pIndex) {
		final Vec2[] vertices = this.mVertices;
		return Vec2Util.isConvex(vertices[pIndex], vertices[pIndex - 1], vertices[pIndex + 1]);
	}
	...
	private void quicksort(final int pFromIndex, final int pToIndex) {
		...
		final Vec2 q = vertices[(pFromIndex + pToIndex) / 2];
		while(i <= j) {
			while(Vec2Util.isLess(vertices[i], q)) {
				i++;
			}
			while(Vec2Util.isLess(q, vertices[j])) {
				j--;
			}
			if(i <= j) {
				this.swap(i++, j--);
