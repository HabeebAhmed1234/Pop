...
public class GameTexturesManager extends BaseEntity {
  ...
  private static final int BACKGROUND_WIDTH_PX = 1080;
  private static final int BACKGROUND_HEIGHT_PX = 1920;
  ...
  private static final List<ImageResource> IMAGE_RESOURCES = Arrays.asList(
      ...
      new ImageResource(TextureId.SAVE_BTN, "pause_button.png", 200, 200)
  );
  ...
  /**
   * Loads the background texture from equipped backgrounds if one exists and sets it to the
   * BACKGROUND pair of mTextureRegions.
   */
  public void initBackgroundTexture() {
    @DrawableRes int equippedBackgroundRes = EquipBackgroundHelper
        .getBackgroundDrawableRes(get(Context.class));
    if (equippedBackgroundRes != -1) {
      BitmapTextureAtlas atlas = createNewAtlas(BACKGROUND_WIDTH_PX, BACKGROUND_HEIGHT_PX);
      ITextureRegion equippedBackgroundTexture = BitmapTextureAtlasTextureRegionFactory
          .createFromResource(atlas, get(Context.class), equippedBackgroundRes, 0, 0);

      mTextureRegions.put(TextureId.BACKGROUND, equippedBackgroundTexture);
    }
  }
