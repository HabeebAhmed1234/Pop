...
class TrayAnimationManager extends BaseEntity {
  ...
  public ListenableFuture openTray(@Nullable SoundId openSound) {
    ...
    if (currentState == State.EMPTY || currentState == State.EXPANDED) {
      return Futures.immediateFailedFuture(
          new IllegalStateException("Cannot open tray in state " + currentState));
    }
  ...
  private ListenableFuture startAnimation(IEntityModifier entityModifier,
      final IModifier.IModifierListener listener) {
    ...
    get(HostTrayCallback.class).getTrayIconsHolderRectangle()
        .registerEntityModifier(currentAnimation);
  ...
  private void stopCurrentAnimation() {
    if (currentAnimation != null) {
      get(HostTrayCallback.class).getTrayIconsHolderRectangle()
          .unregisterEntityModifier(currentAnimation);
    }
  }
  ...
  private IEntityModifier getTrayEntityModifier(int[] destinationPostition,
  ...
  private float getAnimationDuration(float destinationPosition, float currentPosition,
      float totalDelta) {
    return Math.abs(destinationPosition - currentPosition) / Math.abs(totalDelta)
