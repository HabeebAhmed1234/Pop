...
public class BubblePacker {
  ...
  /**
   * Given the number of bubbles we want to spawn returns their initial locations so that they don't
   * spawn on top of one another
   */
  public static List<Pair<Float, Float>> getSpawnBubblesLocations(int numBubbles,
      float bubbleSizePx) {
    ...
    int numBubblesInRow = Math
        .min(numBubbles, (int) Math.floor(ScreenUtils.getSreenSize().widthPx / bubbleSizePx));
    ...
    for (Pair<Float, Float> position : positions) {
      offsetPositions
          .add(new Pair<>(position.first + leftPadding, position.second + bottomPadding));
    }
