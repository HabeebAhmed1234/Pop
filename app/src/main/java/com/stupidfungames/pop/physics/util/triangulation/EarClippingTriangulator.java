 ...
 ******************************************************************************/
package com.stupidfungames.pop.physics.util.triangulation;
...
public final class EarClippingTriangulator implements ITriangulationAlgoritm {
	...
	public List<Vec2> computeTriangles(final List<Vec2> pVertices) {
		...
		final ArrayList<Vec2> triangles = new ArrayList<Vec2>();
		final ArrayList<Vec2> vertices = new ArrayList<Vec2>(pVertices.size());
	...
	private static boolean areVerticesClockwise(final ArrayList<Vec2> pVertices) {
		final int vertexCount = pVertices.size();

		float area = 0;
		for(int i = 0; i < vertexCount; i++) {
			final Vec2 p1 = pVertices.get(i);
			final Vec2 p2 = pVertices.get(EarClippingTriangulator.computeNextIndex(pVertices, i));
	 ...
	 */
	private int[] classifyVertices(final ArrayList<Vec2> pVertices) {
		final int vertexCount = pVertices.size();
		...
		for(int index = 0; index < vertexCount; index++) {
			...
			final Vec2 previousVertex = pVertices.get(previousIndex);
			final Vec2 currentVertex = pVertices.get(index);
			final Vec2 nextVertex = pVertices.get(nextIndex);
	 ...
	 */
	private static boolean isAnyVertexInTriangle(final ArrayList<Vec2> pVertices, final int[] pVertexTypes, final float pX1, final float pY1, final float pX2, final float pY2, final float pX3, final float pY3) {
		int i = 0;

		final int vertexCount = pVertices.size();
		while(i < vertexCount - 1) {
			if((pVertexTypes[i] == CONCAVE)) {
				final Vec2 currentVertex = pVertices.get(i);
...
//				}
	...
	private boolean isEarTip(final ArrayList<Vec2> pVertices, final int pEarTipIndex, final int[] pVertexTypes) {
		if(this.mConcaveVertexCount != 0) {
			final Vec2 previousVertex = pVertices.get(EarClippingTriangulator.computePreviousIndex(pVertices, pEarTipIndex));
			final Vec2 currentVertex = pVertices.get(pEarTipIndex);
			final Vec2 nextVertex = pVertices.get(EarClippingTriangulator.computeNextIndex(pVertices, pEarTipIndex));
	...
	private void cutEarTip(final ArrayList<Vec2> pVertices, final int pEarTipIndex, final ArrayList<Vec2> pTriangles) {
		...
		if(!EarClippingTriangulator.isCollinear(pVertices, previousIndex, pEarTipIndex, nextIndex)) {
			pTriangles.add(new Vec2(pVertices.get(previousIndex)));
			pTriangles.add(new Vec2(pVertices.get(pEarTipIndex)));
			pTriangles.add(new Vec2(pVertices.get(nextIndex)));
		}
	...
	private static void removeCollinearNeighborEarsAfterRemovingEarTip(final ArrayList<Vec2> pVertices, final int pEarTipCutIndex) {
	...
	private static boolean isCollinear(final ArrayList<Vec2> pVertices, final int pIndex) {
	...
	private static boolean isCollinear(final ArrayList<Vec2> pVertices, final int pPreviousIndex, final int pIndex, final int pNextIndex) {
		final Vec2 previousVertex = pVertices.get(pPreviousIndex);
		final Vec2 vertex = pVertices.get(pIndex);
		final Vec2 nextVertex = pVertices.get(pNextIndex);
	...
	private static int computePreviousIndex(final List<Vec2> pVertices, final int pIndex) {
		return pIndex == 0 ? pVertices.size() - 1 : pIndex - 1;
	}

	private static int computeNextIndex(final List<Vec2> pVertices, final int pIndex) {
